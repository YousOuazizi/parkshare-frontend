...
export class VerificationWizardComponent implements OnInit {
    ...
    this.verificationService.getVerificationInfo().subscribe({
      ...
      error: (error) => {
        ...
        this.errorMessage.set(
          "Failed to load verification info. Please refresh the page.",
        );
      },
    });
  }
  ...
  requestEmailVerification(): void {
    ...
    this.verificationService
      .requestEmailVerification(this.emailForm.value)
      .subscribe({
        next: (response) => {
  ...
  requestPhoneVerification(): void {
    ...
    this.verificationService
      .requestPhoneVerification(this.phoneForm.value)
      .subscribe({
        next: (response) => {
          this.loadingStep.set(false);
    ...
    if (file) {
      this.verificationService.uploadDocument(documentType, file).subscribe({
        next: (response) => {
          this.loadingStep.set(false);
          this.successMessage.set(
            "Identity document uploaded successfully! It will be reviewed shortly.",
          );
          this.loadVerificationInfo();
  ...
  uploadAdvancedDocument(): void {
    if (
      this.advancedForm.invalid ||
      !this.selectedAdvancedFile() ||
      this.loadingStep()
    )
      return;
    ...
    if (file) {
      this.verificationService.uploadDocument(documentType, file).subscribe({
        next: (response) => {
          this.loadingStep.set(false);
          this.successMessage.set(
            "Advanced document uploaded successfully! It will be reviewed shortly.",
          );
          this.loadVerificationInfo();
