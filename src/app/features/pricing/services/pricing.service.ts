...
export class PricingService {
  ...
  getPriceRules(parkingId: string): Observable<PriceRule[]> {
    return this.api.get<PriceRule[]>(
      API_ENDPOINTS.PRICE_RULES.BY_PARKING(parkingId),
    );
  }
  ...
  suggestPrice(parkingId: string): Observable<PriceSuggestion> {
    return this.api.post<PriceSuggestion>(API_ENDPOINTS.PRICING.SUGGEST, {
      parkingId,
    });
  }
  ...
  getSuggestionById(id: string): Observable<PriceSuggestion> {
    return this.api.get<PriceSuggestion>(
      API_ENDPOINTS.PRICING.SUGGESTION_BY_ID(id),
    );
  }

  applySuggestion(id: string): Observable<{ message: string }> {
    return this.api.post<{ message: string }>(
      API_ENDPOINTS.PRICING.APPLY_SUGGESTION(id),
      {},
    );
  }
