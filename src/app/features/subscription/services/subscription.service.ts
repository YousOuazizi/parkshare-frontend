...
export class SubscriptionService {
  ...
  getPlans(): Observable<SubscriptionPlan[]> {
    return this.api.get<SubscriptionPlan[]>(
  ...
  updateSubscription(
    ...
    return this.api.patch<Subscription>(
  ...
  shareSubscription(
    ...
    return this.api.post<{ message: string }>(
      API_ENDPOINTS.SUBSCRIPTIONS.SHARE(id),
      { email },
    );
  }
  ...
  acceptShare(shareId: string): Observable<Subscription> {
    return this.api.post<Subscription>(
      API_ENDPOINTS.SUBSCRIPTIONS.ACCEPT_SHARE(shareId),
      {},
    );
  }
  ...
  rejectShare(shareId: string): Observable<void> {
    return this.api.post<void>(
      API_ENDPOINTS.SUBSCRIPTIONS.REJECT_SHARE(shareId),
      {},
    );
  }
  ...
  revokeShare(shareId: string): Observable<void> {
    return this.api.post<void>(
      API_ENDPOINTS.SUBSCRIPTIONS.REVOKE_SHARE(shareId),
      {},
    );
  }

  getUsageReport(id: string): Observable<SubscriptionUsageReport> {
    return this.api.get<SubscriptionUsageReport>(
      API_ENDPOINTS.SUBSCRIPTIONS.USAGE(id),
    );
  }

  checkAccess(
    parkingId: string,
  ): Observable<{ hasAccess: boolean; subscription?: Subscription }> {
